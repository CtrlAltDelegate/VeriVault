...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = path.join(__dirname, '../../../uploads/daily-logs');
...
router.post('/submit', upload.array('attachments'), async (req: Request, res: Response) => {
  try {
    ...
    const uploadedFiles = req.files as multer.File[];
    ...
    const dailyLogEntry = {
      ...
      summary: {
        ...
        patrolRoundsCompleted: parsedFormData.patrolRounds?.filter((r: any) => r.time).length || 0,
...
router.get('/attachment/:submissionId/:filename', (req, res) => {
  ...
  const attachment = log.attachments.find((att: any) => att.filename === filename);
  if (!attachment) {
    return res.status(404).json({
      success: false,
...
router.delete('/:id', async (req, res) => {
  ...
  if (log.attachments && log.attachments.length > 0) {
    log.attachments.forEach((attachment: any) => {
      try {
        if (fs.existsSync(attachment.uploadPath)) {
          fs.unlinkSync(attachment.uploadPath);
        }
      } catch (error) {
