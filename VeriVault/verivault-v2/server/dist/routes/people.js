...
router.get('/', (req, res) => {
    ...
    // Filter by type
    if (type && type !== 'all') {
        filteredPeople = filteredPeople.filter(person => person.type.toLowerCase() === type.toLowerCase());
    }
    // Search by name or company
    if (search) {
        const searchTerm = search.toLowerCase();
        filteredPeople = filteredPeople.filter(person => `${person.firstName} ${person.lastName}`.toLowerCase().includes(searchTerm) ||
    ...
    // Sort by name
    filteredPeople.sort((a, b) => `${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`));
    res.json({
        success: true,
...
router.post('/', (req, res) => {
    ...
    const existingPerson = people.find(person => person.firstName.toLowerCase() === firstName.toLowerCase() &&
...
router.get('/search/:query', (req, res) => {
    ...
    const searchResults = people
        .filter(person => `${person.firstName} ${person.lastName}`.toLowerCase().includes(query) ||
