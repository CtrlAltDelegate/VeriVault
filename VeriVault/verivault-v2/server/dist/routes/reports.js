...
const storage = multer_1.default.diskStorage({
    destination: (req, file, cb) => {
        const uploadsDir = path_1.default.join(__dirname, '../../uploads/reports');
...
router.post('/generate', upload.array('attachments'), async (req, res) => {
    try {
...
function generateMedicalIncidentPDF(formData, verificationData, submissionId, attachments) {
    const timestamp = new Date().toISOString();
...
function generateNonMedicalIncidentPDF(formData, verificationData, submissionId, attachments) {
    const timestamp = new Date().toISOString();
    const watermark = `VERIFIED - PIN AUTHENTICATED - ${verificationData.username.toUpperCase()} - ${timestamp}`;
    return `
%PDF-1.4
...
function generateSecurityAuditPDF(formData, verificationData, submissionId, attachments) {
    const timestamp = new Date().toISOString();
    const watermark = `VERIFIED - PIN AUTHENTICATED - ${verificationData.username.toUpperCase()} - ${timestamp}`;
    return `
%PDF-1.4
